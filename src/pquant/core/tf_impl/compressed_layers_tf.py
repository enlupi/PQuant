...
def set_quantization_bits_weight_layers(config, layer, new_layer):
    ...
    if isinstance(layer, SeparableConv2D):
        dw_i_bits_w = pw_i_bits_w = pw_i_bits_b = i_input = i_output = config["quantization_parameters"][
            "default_integer_bits"
        ]
        dw_f_bits_w = pw_f_bits_w = pw_f_bits_b = f_input = f_output = config["quantization_parameters"][
